...
def trained_tagger(existing=False):
    ...
    # Create instance of Subjec--tTrigramTagger and persist instance of it
...
if __name__ == '__main__':
    url = 'http://www.bbc.com/news/world-asia-39768110'
    document = download_document(url)
    # document = pickle.load(open('document.pkl', 'rb'))
    print (document)
    document = clean_document(document)
    subject = extract_subject(document)
    print (subject) #this is the most comonly used subject

    tagged_sents = tag_sentences(subject, document)
    svos = [get_svo(sentence, subject)
                        for sentence in tagged_sents] #gets subject verb object for usages of this heftly used subject.
    for svo in svos:
        if svo:
            print (svo)

#this should honestly be all i need to figure out the topic of an article, I don't really need to summarize the article? But I do need to know whether the article is talking positively or negatively
#about the topic. If for example